        ...
        if ($action == 'assign') {
            ...
            if ( !$this->input->get('view') ) {                
                ...
                foreach($results as $key => $class ){
                    $match = array_filter($results, function($item) use ($class, &$exclude) {
                            
                        if ($class->id == $item->partner_id && !in_array($class->id, $exclude)) {
                            $exclude[] = $class->id;
                            return true;
                        }
                    });
                    ...
                    $partnerships[$i] = $match;
                    $i++;                    
                }
                
                $this->data['partnerships'] = $partnerships;

                $query = $this->db->query("SELECT classes.* FROM classes WHERE course_id = " . $id . " AND id NOT IN (". implode(',', $exclude) .")");
