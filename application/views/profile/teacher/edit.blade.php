...
<script type="text/javascript" charset="utf-8">
    $(document).ready(function() {                                           
        ...
        $("#filepicker").on('change', function(event) {
            ...
            $.get('/teacher/profile/change_avatar', {avatar : file, custom: true}, function(data) {
        ...
        $('.avatar').on('click', function(event) {
            ...
            $.get('/teacher/profile/change_avatar', {avatar : avatar, custom: false}, function(data) {
               window.location.reload(); // reload page
            })            
        });
        
        $('#opt_in_email').on('click', function(event) {
            event.preventDefault();
            var opt_in_email = $(this).attr('checked');

            opt_in_email = ( opt_in_email == 'checked' )? 1 : 0;
            $.get('/teacher/profile/set_email_preference', {opt_in_email : opt_in_email}, function(data) {
               window.location.reload(); // reload page
            })                        
        });
    });
	...
	<div class="profile_container">
		<div class="left w40">
            ...
            <div class="profile-box w100">
                ...
                <input id="opt_in_email" type="checkbox" @if($user->opt_in_email) checked="checked" @endif value="{{$user->opt_in_email}}" name="opt_in_email">
                <label>receive the PenPal News Newsletter</label>
            </div>
			<!-- end Join Session -->
		...
		<div class="right w50">
			<div class="profile-box w100">
				...
				<?php echo $this->formbuilder->open( '/teacher/profile/save', array('id'=>'join_profile')) ?>    	            
			...
			<div class="profile-box w100">
				...
				<div class="left w50">
					<ul>
						<li>
							<label>Name:</label>
							<p class="edit">{{$user->first_name}}&nbsp;{{$user->last_name}}<br/></p>
						</li>
						<li>
